...
candidate Release physical qualification are complete for the available
single-monitor desktop. The evidence records are being closed for the final
post-documentation Release rebuild, authorized mainline push, and independent
...
Close the evidence records, inspect the final diff, commit only campaign
files, publish and exercise the exact post-commit Release artifact, then push
`main` without force and independently verify remote SHA and CI status. The
