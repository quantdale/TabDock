...
- Historical qualified RC bundle: `artifacts\rc-candidate\TabDock.exe` +
  ...
  fixes; diagnostic only). The final delivery bundle is generated by
  `scripts\release-qualify.ps1` under the ignored
  `.artifacts\mainline-release-closure-<final-head>\` lineage after the last
  committed-tree gate and is verified against that exact source identity.
...
strict OpenSpec 34/34; canonical Release validation/publish PASS. The final
delivery action generates a fresh exact-SHA candidate and qualification bundle
from the committed tree; the prior candidate remains historical diagnostic
evidence only.
