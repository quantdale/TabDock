...
- Closed container HWNDs are never unregistered from `GroupManager._ownContainerHwnds` — both reads (`WindowInteropHelper.Handle`, `NativeHwndHost.HostWindowHandle`) return `IntPtr.Zero` by `Closed` time (`Views/ContainerWindow.xaml.cs:304-307`), so every closed container leaks two HWNDs into the set. When Windows recycles one of those values for an unrelated window, `IsOwnWindow` (`Services/GroupManager.cs:129`) refuses to capture it as "no nesting".
...
- `Hide()` (`:186-192`) and `Release(show:false)` (`:271-281`) write the crash-recovery journal *after* `SW_HIDE`; a force-kill in between leaves an unrescuable hidden orphan (or a stale entry rescue wrongly un-shows).
