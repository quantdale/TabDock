...
- [x] 1.1 Add `App.ReconcileRestoredContainerZOrder()` that raises each
...
- [x] 1.2 Call it from `Application_Startup` immediately after the
      restored-container `foreach`, before `SyncWinEventMonitor()`/the
      "TabDock startup complete." log.

## 2. Regression scenarios

- [x] 2.1 Add `startup-group-not-hidden-behind-existing-window` (reproduction):
...
- [x] 2.2 Add `startup-does-not-steal-foreground-after-external-activation`
      (guard): after startup settles, activate the blocker and assert TabDock
      does not re-take foreground. Register similarly.
- [x] 2.3 Add `startup-local-stack-above-unrelated-when-guest-present` (guard):
...
- [x] 3.1 Build `TabDock.csproj`, `TabDock.sln`, ValidationDriver, GuineaPig,
      and Spike with 0 warnings / 0 errors.
- [x] 3.2 Run `scripts/validate.ps1`, `TabDock.exe --selftest-geometry`,
      `openspec validate --all --no-interactive`, and `git diff --check`.

## 4. Durable state / docs

- [x] 4.1 Update `.agent/STATE.md`, `docs/internal/whole-codebase-audit-waypoint.md`,
...
## 5. Manual acceptance (must remain explicit)

- [x] 5.1 Run the supervised startup visibility scenarios on an interactive
      Windows desktop; the three registered startup scenarios passed on
      2026-08-13.
- [ ] 5.2 Complete the human visual checklist on an interactive Windows
      desktop; do not mark this complete from hosted CI or CLI-only validation.
