...
- [x] 1.1 Add `PersistenceBackupDurabilityTests` with injected-seam fault tests:
...
- [x] 1.2 Cover: valid `.bak` survives failed next-backup creation; missing
...
- [x] 1.3 Add pending-recovery regression matrix: resolution liveness bounding,
      ...
      preservation.
- [x] 1.4 Re-anchor the tests that pinned the old orphan-sidecar behavior on
...
- [x] 2.1 PersistenceService: staged durable backup candidate + atomic install
...
- [x] 2.2 PendingRecoveryService: `CompactUnreachableResolutions` invoked from
...
- [x] 3.1 persistence-resilience delta: ADDED requirement for the staged backup
      transaction with failure-window scenarios.
- [x] 3.2 hidden-window-journal delta: ADDED requirement for bounded resolution
      bookkeeping and orphaned sidecar retirement with convergence scenarios.
- [x] 3.3 `openspec validate --all --no-interactive` passes.

## 4. Qualification

- [x] 4.1 Debug + Release builds 0 warnings / 0 errors.
- [x] 4.2 xUnit suite green in Debug and Release; report exact counts.
- [x] 4.3 `scripts/release-tooling-tests.ps1` green; `scripts/validate.ps1
      -Configuration Release -Ci -Publish` PASS.
- [x] 4.4 `git diff --check` clean; STATE.md handoff updated with next queued
