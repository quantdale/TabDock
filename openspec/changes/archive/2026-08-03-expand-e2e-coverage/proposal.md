...
- **Bug-hunt-derived flows with no automated coverage today** — three more Session-4 static-audit fixes (`KNOWN_ISSUES.md`, 2026-07-25) that are "reviewed-and-reasoned, not runtime-confirmed" and aren't exercised by any existing scenario: (1) a restored group's persisted layout intent must survive its *re-captured* live member being destroyed or tray-hidden again, not just an empty-shell clean exit (`persist-kill` only covers the latter — `RemoveDeadMember`'s own `PersistedTabs.Count == 0` guard, `App.xaml.cs:365`, is untested); (2) a guest that minimizes itself and then has its tab released or its container torn down before the 200ms `RestoreMinimizedWindow` timer fires must not get spuriously restored/repositioned by that stale timer; (3) the launcher's "No groups yet" hint must be visible with zero groups and hidden once one exists.
- **Safety discipline as spec** — the four input-injection rules from `docs/TESTING.md` (identity verification immediately before input, scope to owned windows, unconditional try/finally cleanup, single-run-then-report with re-verification) become explicit requirements every new or modified scenario must satisfy, plus a fifth: every scenario must assert zero orphaned guest windows survive its own run (formalizing the existing `NoOrphanPigWindows` helper, today only invoked by a couple of scenarios, into a rule all of them follow).
...
- **Code**: `tests/ValidationDriver/TabDock.ValidationDriver/Scenarios.cs` (scenario edits/additions), possibly `Program.cs` (scenario registration/`AllOrder`), and `TabDock.GuineaPig` only if a scenario needs a new guest switch (goal: none). The launcher hint scenario reads `Views/MainWindow.xaml`'s UIA tree only — no production edits.
