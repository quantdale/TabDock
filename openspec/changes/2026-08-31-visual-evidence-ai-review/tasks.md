...
> **Ledger reconciliation (2026-09-01).** This canonical 178-row ledger was
> reconciled against current source, tests, documentation, workflows, and
> inspected run artifacts. Exactly **93** rows are checked as
> `COMPLETED_AND_PROVEN`; **85** remain unchecked with explicit classifications
> in `.agent/investigations/visual-evidence-ledger-reconciliation-2026-09-01.md`.
> The historical `84/89` implementation roll-up was not a row-level mapping and
> MUST NOT be treated as a canonical checkbox count. No archival or commit/push
> claim is made by this reconciliation.
## 0. Orientation and scope lock

- [x] 0.1 Resolve current `HEAD`, `origin/main`, branch and worktree dynamically before implementation; do not rely on SHAs embedded in this plan.
- [x] 0.2 Read `AGENTS.md`, `.agent/STATE.md`, `docs/TESTING.md`, the active presentation-integrity physical-certification change, the canonical qualification specs, and current ValidationDriver artifact code before editing.
- [x] 0.3 Inventory all current pixel-capture callers, artifact writers, run manifests, bundle verifiers, timeline records, and cleanup paths.
- [x] 0.4 Confirm this campaign changes validation/evidence infrastructure only. Do not change production TabDock behavior unless a separate valid defect investigation proves a product change is necessary.
- [x] 0.5 Record a concise implementation plan/investigation checkpoint under `.agent/` and link it from `.agent/STATE.md`.

## 1. Current-capture baseline

- [x] 1.1 Characterize `Pixels.CaptureHostScreenArea` and `CaptureWindowViaPrintWindow` on the current controlled GuineaPig fixtures.
- [x] 1.2 Verify channel order, dimensions, screen/client coordinate translation, DWM-composited semantics, and failure behavior.
- [x] 1.3 Measure capture latency and raw memory for representative small/default/large windows.
- [x] 1.4 Confirm which existing scenarios already rely on brightness, frame diff, dominant color, screen pixels, or PrintWindow so the new layer preserves their semantics.
- [x] 1.5 Add regression tests before refactoring any shared pixel code.

## 2. Visual evidence domain model

- [x] 2.1 Add versioned types for `VisualFrame`, `VisualCaptureScope`, `VisualCheckpointPhase`, `VisualCheckpointRequest`, `VisualArtifactRecord`, and `VisualEvidencePolicy`.
- [x] 2.2 Define stable enums/IDs for capture method, scope, privacy class, evidence level, checkpoint phase, and required-vs-optional capture.
- [x] 2.3 Ensure all paths are normalized relative paths below the run-owned artifact root.
- [x] 2.4 Ensure target identity metadata uses existing run-safe process/HWND identity conventions rather than inventing a weaker title-based identity.
- [x] 2.5 Unit-test serialization, invalid enums, malformed rectangles, empty dimensions, and unsafe paths.

## 3. PNG encoding and immutable artifacts

- [x] 3.1 Implement lossless PNG encoding for the current 32-bit pixel representation without introducing a large new imaging dependency unless proven necessary.
- [x] 3.2 Add deterministic tests proving red/green/blue channel correctness and exact width/height preservation.
- [x] 3.3 Write images through a bounded temporary path and atomic finalization under the attempt artifact root.
- [x] 3.4 Compute SHA-256 over the final PNG bytes and store size/MIME/hash in the artifact record.
- [x] 3.5 Reject absolute paths, traversal, duplicate artifact IDs, duplicate output paths, and writes outside the artifact root.
- [x] 3.6 Make registered raw PNGs immutable from the recorder's perspective; derived annotations/contact sheets use separate files.
- [x] 3.7 Add cleanup behavior for partial temp files after exceptions/cancellation.
...
- [x] 5.1 Add one scenario-facing `ctx.Visual`-style API so scenarios request semantic checkpoints rather than files directly.
- [x] 5.2 Support `Baseline`, `BeforeAction`, `AfterActionImmediate`, `AfterActionSettled`, `BeforeAssertion`, `Suspicious`, `AssertionFailure`, `FinalHealthy`, and `BeforeCleanup` phases.
- [x] 5.3 Require every retained checkpoint to include a stable checkpoint ID and explicit visual expectation.
- [x] 5.4 Support multiple approved scopes per checkpoint without duplicating scenario plumbing.
- [x] 5.5 Distinguish required capture from best-effort capture.
- [x] 5.6 If required capture fails, surface `FAIL_HARNESS`; never silently continue to a visual PASS.
- [x] 5.7 If optional capture fails, retain explicit unavailable metadata without relabeling native results.
- [x] 5.8 Add artifact count and byte ceilings; prove the recorder stops accepting additional optional evidence after the limit rather than growing unbounded.
- [x] 5.9 Record capture/encode duration and retained byte counters.
...
- [x] 10.1 Implement deterministic chronological contact-sheet generation for selected checkpoints.
...
- [x] 12.1 Define and version `tabdock-visual-review-result-v1`.
- [x] 12.2 Support verdicts `VISUAL_OK`, `VISUAL_SUSPECT`, `VISUAL_DEFECT`, and `REVIEW_UNAVAILABLE`.
- [x] 12.3 Define finding categories for occlusion, blank/black region, wrong guest, clipping, misalignment, popup placement, z-order composition, transient flicker/flash, stale frame, visible geometry drift, DPI anomaly, unexpected chrome, capture artifact, and other.
- [x] 12.4 Require every concrete finding to reference checkpoint/artifact IDs and the exact reviewed image hashes.
...
- [x] 13.1 Add one harness-neutral canonical workflow under `.agent/workflows/` for visual-evidence review.
- [x] 13.2 Tell the agent to validate the packet first, open the contact sheet, then inspect suspicious raw frames at full resolution.
- [x] 13.3 Tell the agent to describe visible symptoms before hypothesizing causes.
- [x] 13.4 Tell the agent to correlate findings with HWND/UIA/pixel/timeline evidence before authorizing source changes.
...
- [x] 20.1 Add privacy classes `TestOwned`, `ProductOwned`, `RealAppRestricted`, and `DesktopRestricted` or equivalent.
...
- [x] 22.8 Run ValidationDriver self-tests/catalog validation.
- [x] 22.9 Run `scripts/validate.ps1 -Configuration Release -Ci -Publish`.
- [x] 22.10 Run strict OpenSpec validation.
