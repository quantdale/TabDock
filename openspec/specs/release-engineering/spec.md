...
self-test SHALL be the same binary that is hashed, manifested, and available
for publication. Hermetic behavioral qualification is owned by the xUnit
suite, which compiles against the same product sources in the same CI gate.
