...
**The WinEvent path is the app's hot path, and it is optimized on purpose.** `GroupManager` keeps a dictionary index of every group's members, maintained from `Group.Members`' `CollectionChanged` (so no capture/release path has to remember it); `IsCapturedWindow` and `TryGetCapturedMember` are O(1) probes into it, and both the monitor's filter and every handler in `GuestLifecycleService` (the single consumer of `WinEventMonitor` events, wired in via one `Attach` call) go through them — do not reintroduce a `Groups.ToList()` + `FirstOrDefault` scan into an event handler, and do not mutate the index by hand. The hooks themselves are installed only while at least one window is captured (`GroupManager.MonitoringNeededChanged` → `App.SyncWinEventMonitor`), because a hook set that spans the whole desktop is pure cost when every handler would discard the event anyway. See `docs/internal/perf-2026-07-25.md` (PERF25-02/03) plus the "Performance-sensitive paths" section of `AGENTS.md` before changing any of this.
...
## Historical bug/investigation logs

Before touching z-order (`SetWindowPos`), activation (`WM_ACTIVATE`), or `ValidationDriver` scenario code, skim `KNOWN_ISSUES.md` and `investigation_findings.md` — they document specific, previously-shipped bugs in exactly these areas (e.g. a `SetWindowPos` `hwndInsertAfter` direction reversal, and a guest self-hide racing the container's `WM_ACTIVATE` handler) with root causes and the fixes applied, so the same mistakes aren't reintroduced. `docs/internal/TEST_PLAN.md` describes the ValidationDriver's real-browser regression test design if you're adding scenarios there.
