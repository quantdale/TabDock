...
internal static class VirtualTopologyPolicy
{
    ...
    public static LabRect Project(LabRect rectangle, LabRect fromWorkArea, LabRect toWorkArea)
    {
        ...
        double xRatio = (rectangle.Left - fromWorkArea.Left) / (double)Math.Max(1, fromWorkArea.Width - rectangle.Width);
        double yRatio = (rectangle.Top - fromWorkArea.Top) / (double)Math.Max(1, fromWorkArea.Height - rectangle.Height);
    ...
    private static LabMonitor? FindMonitorForRectangle(LabRect rectangle, LabTopology topology)
    {
        int x = rectangle.Left + Math.Max(0, rectangle.Width / 2);
        int y = rectangle.Top + Math.Max(0, rectangle.Height / 2);
        return topology.Monitors.FirstOrDefault(monitor => monitor.WorkArea.ContainsPoint(x, y));
    }
}
...
internal static class VirtualTopologyLab
{
    ...
    public const string Generation = "virtual-topology-lab-2026-08-24-v1";
    public const int Seed = 20260824;

    public static VirtualTopologyLabReport Run()
    {
        var cases = new List<VirtualTopologyLabCase>();
        foreach (LabTopology topology in FixedTopologies())
            cases.Add(EvaluateTopology(topology));

        int randomAssertions = 0;
        string? randomFailure = null;
        var random = new Random(Seed);
        LabTopology[] matrix = FixedTopologies().ToArray();
        for (int iteration = 0; iteration < 256; iteration++)
        {
            LabTopology topology = matrix[random.Next(matrix.Length)];
            ...
            LabRect desired = new(
                monitor.WorkArea.Left - random.Next(0, 2000),
                monitor.WorkArea.Top - random.Next(0, 2000),
                monitor.WorkArea.Left - random.Next(0, 2000) + width,
                monitor.WorkArea.Top - random.Next(0, 2000) + height);
            ...
            if (left.Right != right.Left || left.Left != monitor.WorkArea.Left || right.Right != monitor.WorkArea.Right)
            {
                randomFailure = $"partition failed at iteration {iteration} seed {Seed}";
                break;
            }
            LabTopology nextTopology = matrix[(iteration + 1) % matrix.Length];
            LabRect projected = VirtualTopologyPolicy.RestoreAfterTransition(contained, topology, nextTopology);
            if (!nextTopology.Monitors.Any(monitor => monitor.WorkArea.Contains(projected)))
            {
        ...
        else
        {
            cases.Add(new VirtualTopologyLabCase(
                "seeded-transition-stress",
                matrix.Length,
                matrix.SelectMany(topology => topology.Monitors.Select(monitor => monitor.EffectiveDpi)).Distinct().OrderBy(value => value).ToArray(),
                matrix.Any(topology => topology.HasNegativeCoordinates),
                matrix.Any(topology => topology.HasAboveOriginMonitor),
                true,
                randomAssertions,
                null));
        }
        ...
        return new VirtualTopologyLabReport(
            ...
            cases.Sum(item => item.AssertionCount),
            hash,
            cases);
    }
    ...
    public static IReadOnlyList<LabTopology> FixedTopologies()
    {
        return new[]
        {
            ...
            new LabTopology("mixed-100-125-150-200", "m1", new[]
            {
                ...
                new LabMonitor("m4", new LabRect(3200, -400, 5120, 1040), new LabRect(3200, -400, 5120, 1000), 192),
            }),
    ...
    private static VirtualTopologyLabCase EvaluateTopology(LabTopology topology)
    {
        ...
        foreach (LabMonitor monitor in topology.Monitors)
        {
            ...
            if (!monitor.WorkArea.Contains(dragged))
        ...
        LabRect restored = VirtualTopologyPolicy.RestoreAfterTransition(
            new LabRect(topology.Primary.WorkArea.Left + 10, topology.Primary.WorkArea.Top + 10,
        ...
        return new VirtualTopologyLabCase(
            ...
            topology.Monitors.Select(monitor => monitor.EffectiveDpi).OrderBy(value => value).ToArray(),
